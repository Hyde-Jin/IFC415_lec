import cv2 as cv
import numpy as np
import threading, time
import SDcar 

def func_thread():
    i = 0
    while True:
        # print("alive!!")    
        time.sleep(1)
        i = i + 1
        if is_running is False:
            break

def key_cmd(which_key):
    print('which_key', which_key)
    is_exit = False
    global enable_linetracing
    if which_key & 0xFF == 82:
        car.motor_go(100)
    elif which_key & 0xFF == 84:
        car.motor_back(100)
    elif which_key & 0xFF == 81:
        car.motor_left(100)
    elif which_key & 0xFF == 83:
        car.motor_right(100)            
    elif which_key & 0xFF == 32:
        car.motor_stop()
        print('stop')
    elif which_key & 0xFF == ord('q'):  
        car.motor_stop()
        print('exit')        
        is_exit = True
    elif which_key & 0xFF == ord('e'):
        enable_linetracing = True
        print('enable_linetracing : ', enable_linetracing)
    elif which_key & 0xFF == ord('w'):
        enable_linetracing = False
        car.motor_stop()
        print('enable_linetracing 2: ', enable_linetracing)
    return is_exit  

def detect_maskY_HSV(frame):
    crop_hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
    crop_hsv = cv.GaussianBlur(crop_hsv, (5, 5), cv.BORDER_DEFAULT)

    mask_Y = cv.inRange(crop_hsv, (25, 50, 100), (35, 255, 255))
    return mask_Y

def detect_white_lines_BGR(frame):
    hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
    
    yellow_lower = np.array([20, 100, 100])
    yellow_upper = np.array([40, 255, 255])
    yellow_mask = cv.inRange(hsv, yellow_lower, yellow_upper)
    
    kernel_dilate = np.ones((20,20), np.uint8)
    yellow_mask_dilated = cv.dilate(yellow_mask, kernel_dilate, iterations=1)
    
    track_interior = cv.bitwise_not(yellow_mask_dilated)
    
    gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
    _, white_mask = cv.threshold(gray, 220, 255, cv.THRESH_BINARY)
    
    white_lines = cv.bitwise_and(white_mask, track_interior)
    
    kernel_morph = np.ones((5,5), np.uint8)
    white_lines = cv.dilate(white_lines, kernel_morph, iterations=5)
    white_lines = cv.erode(white_lines, kernel_morph, iterations=1)
    
    return white_lines

def line_tracing(cx):
    # print('cx : ', cx)
    # pirnt('v_x_grid : ', v_x_grid)
    global moment
    global v_x
    tolerance = 0.1
    diff = 0

    if moment[0] != 0 and moment[1] != 0 and moment[2] != 0:
        avg_m = np.mean(moment)
        diff = np.abs(avg_m - cx) / v_x
    
    #print('diff = {:.4f}' .format(diff))

    if diff <= tolerance:
        moment[0] = moment[1]
        moment[1] = moment[2]
        moment[2] = cx

        if cx < v_x_grid[2]:
            car.motor_left(speed)
        elif v_x_grid[2] <= cx < v_x_grid[6]:
            car.motor_go(speed)
        elif v_x_grid[6] <= cx:
            car.motor_right(speed)
        else:
            # 선을 벗어난 경우
            car.motor_go(speed)
            print('go')
            moment = [0, 0, 0]

def show_grid(img):
    h, _, _ = img.shape
    for x in v_x_grid:
        # print('show_gird', x)
        cv.line(img, (x, 0), (x, h), (0, 255, 0), 1, cv.LINE_4)

def main():
    camera = cv.VideoCapture(0)
    camera.set(cv.CAP_PROP_FRAME_WIDTH,v_x) 
    camera.set(cv.CAP_PROP_FRAME_HEIGHT,v_y)

    try:
        while(camera.isOpened()):
            ret, frame = camera.read()
            frame = cv.flip(frame, -1)
            cv.imshow('camera', frame)

            crop_img = frame[120:, 80:240]
            
            white_lines = detect_white_lines_BGR(crop_img)
            
            contours, _ = cv.findContours(white_lines, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)

            if len(contours) > 0:
                c = max(contours, key=cv.contourArea)
                m = cv.moments(c)

                cx = int(m['m10']/(m['m00'] + epsilon))
                cy = int(m['m01']/(m['m00'] + epsilon))
                cv.circle(crop_img, (cx, cy), 3, (0, 255, 0), 1)

                cv.putText(crop_img, str(cx), (10, 10), cv.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 0))

                if enable_linetracing == True:
                    line_tracing(cx)
                    
            show_grid(frame)

            cv.imshow('white_lines', white_lines)
            cv.imshow('crop_img', cv.resize(crop_img, dsize=(0, 0), fx=2, fy=2))
            
            # image processing end here
            is_exit = False
            which_key = cv.waitKey(20)
            if which_key > 0:
                is_exit = key_cmd(which_key)    
            if is_exit is True:
                cv.destroyAllWindows()
                break
    except Exception as e:
        print(e)
        global is_running
        is_running = False

if __name__ == '__main__':
    epsilon = 0.01
    speed = 10
    
    v_x = 320
    v_y = 240
    v_x_grid = [int(v_x  * i / 20) for i in range(1, 20)]

    moment = np.array([0, 0, 0])

    print(v_x_grid)

    t_task1 = threading.Thread(target = func_thread)
    t_task1.start()

    car = SDcar.Drive()
    
    is_running = True
    enable_linetracing = False
    main() 
    is_running = False
    car.clean_GPIO()
    print('end vis')